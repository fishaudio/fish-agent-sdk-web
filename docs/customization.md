...
`language` accepts an ISO 639-1 code from the supported set — `en`, `ja`, `zh`, `ko`, `es`, `fr`, `de`, `pt`, `it`, `nl`. Anything else (language names like `"Chinese"`, endonyms like `"中文"`, region variants like `"zh-CN"`) fails session creation with HTTP 422; values are never best-effort-mapped. Omit it to use the agent's configured speaking language.
